    ...
    return userData ? (
        <div className='max-w-lg flex flex-col gap-2 text-sm pt-5'>
            ...
            {isEdit
                ...
                : <img className='w-full h-48 rounded object-cover' src={userData.image} alt="" />
            }
            ...
            <div>
                <p className='text-gray-600 underline mt-3 text-xl'>CONTACT INFORMATION</p>
                <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-[#363636]'>
                    <p className='font-medium text-lg'>Email id:</p>
                    <p className='text-blue-500 text-lg'>{userData.email}</p>
                    ...
                    {isEdit
                        ...
                        : <p className='text-blue-500 text-lg'>{userData.phone}</p>
                    }

                    <p className='font-medium text-lg'>Address:</p>

                    {isEdit
                        ? <input className='bg-gray-50 text-lg' type="text" onChange={(e) => setUserData(prev => ({ ...prev, address: e.target.value }))} value={userData.address} />
                        : <p className='text-gray-500 text-lg'>{userData.address}</p>
                    }

                </div>
            </div>
            <div>
                <p className='text-[#797979] underline mt-3 text-xl'>BASIC INFORMATION</p>
                <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-gray-600'>
                    ...
                    {isEdit
                        ? <select className='max-w-20 bg-gray-50' onChange={(e) => setUserData(prev => ({ ...prev, gender: e.target.value }))} value={userData.gender} >
                            <option value="Not Selected text-lg">Not Selected</option>
                            <option value="Male">Male</option>
                            <option value="Female">Female</option>
                        </select>
                        : <p className='text-gray-500 text-lg'>{userData.gender}</p>
                    }
                    ...
                    {isEdit
                        ...
                        : <p className='text-gray-500 text-lg'>{userData.dob}</p>
                    }
